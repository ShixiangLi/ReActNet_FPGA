...
#pragma HLS ALLOCATION instances=quant_and_pack limit=1 function
	...
	FIX_FM_acc fmap_input[3][WIDTH][WIDTH];
	uint64 fmap[3][WIDTH][WIDTH];
#pragma HLS ARRAY_PARTITION variable=fmap_input complete dim=1
#pragma HLS ARRAY_PARTITION variable=fmap complete dim=1
	...
	FIX_FM_acc out_buf_t0_input[CHANNEL_OUT_T][WIDTH][WIDTH];
...
#pragma HLS PIPELINE
    ...
    int out_channels, out_channel_start, padding, stride, conv_weight_ptr, conv_input_ptr, conv_downsample_2_ptr, conv_downsample_3_ptr;
	...
	for (int c = 0; c < 3; c ++) {
		for (int row = 0; row < 32; row ++) {
			for (int col = 0; col < 32; col ++) {
				//#pragma HLS PIPELINE
				fmap_input[c][row][col] = image[c][row][col];
			}
		}
	}
    ...
    for (int c_out = 0; c_out < out_channels/OUT_CHANNEL_PARALLELISM; c_out ++){
        ...
        {
            fp_conv3x3_tile(
            		fmap_input[c_in], conv_weight_fp_3x3[conv_input_ptr],
                    out_buf_t0_input,
                    c_in, in_channels, H_fmap_in, stride, padding
            );
            conv_weight_ptr += 1;
        }

        bn1(
        		out_buf_0, out_buf_t0_input,
	...
	////////////////////////////////////////////////
	//////////// layer1_0 PG1 /////////////////////
	quant_and_pack(out_buf_0, fmap, H_fmap_in, in_channels);
	LOOP_layer1_0_PGConv1:
	for (int c_out = 0; c_out < out_channels/OUT_CHANNEL_PARALLELISM; c_out ++) {
		int c_in = 0;
		binary_conv3x3_tile(
        		fmap[c_in], conv_weight_all[conv_weight_ptr],
				out_buf_t0,
                c_in, in_channels, H_fmap_in, stride, padding
		);
		conv_weight_ptr += 1;
	...
	stride = 2;
	padding = 1;
	conv_downsample_2_ptr = 0;

//	FIX_FM_acc layer_2_downsample[in_channels][WIDTH][WIDTH];
	FIX_FM_acc layer_2_downsample_out[CHANNEL_OUT/CHANNEL_OUT_T][CHANNEL_OUT_T][WIDTH][WIDTH];
...
//			for (int c = 0; c < CHANNEL_OUT/CHANNEL_OUT_T; c ++){
//				for (int k = 0; k < CHANNEL_OUT_T; k ++) {
//					layer_2_downsample[c*CHANNEL_OUT + k][i][j] = out_buf_0[c][k][i][j];
//				}
//			}
//		}
//	}

	avgpool_concat(layer_2_downsample_out, out_buf_0, H_fmap_out, in_channels);
	LOOP_layer2_0_PGConv1:
	for (int c_out = 0; c_out < out_channels/OUT_CHANNEL_PARALLELISM; c_out ++) {
		for (int c_in = 0; c_in < in_channels; c_in++)
		{
			fp_conv1x1_tile(
					layer_2_downsample_out[c_out][c_in], downsample2_fp_1x1[conv_downsample_2_ptr][c_in],
					out_buf_t0_input,
	                c_in, in_channels, H_fmap_in, stride, padding
			);
			conv_weight_ptr += 1;
		...
		bn1(
				layer_2_downsample_out, out_buf_t0_input,

				layer2_downsample_bn_weight_fix[c_out],
				layer2_downsample_bn_bias_fix[c_out],
	...
	for (int c_out = 0; c_out < out_channels/OUT_CHANNEL_PARALLELISM; c_out ++) {
		...
		conv_weight_ptr += 1;
		bn_relu_shortcut(
				layer_2_downsample_out, out_buf_t0,
...
#pragma HLS PIPELINE
			for (int c = 0; c < CHANNEL_OUT/CHANNEL_OUT_T; c ++){
				for (int k = 0; k < CHANNEL_OUT_T; k ++) {
					out_buf_0[c][k][i][j] = layer_2_downsample_out[c][k][i][j];
				}
			}
		}
	}
	...
	stride = 2;
	padding = 1;
	conv_downsample_3_ptr = 0;

//	FIX_FM_acc layer_3_downsample[in_channels][WIDTH][WIDTH];
	FIX_FM_acc layer_3_downsample_out[CHANNEL_OUT/CHANNEL_OUT_T][CHANNEL_OUT_T][WIDTH][WIDTH];
...
//			for (int c = 0; c < CHANNEL_OUT/CHANNEL_OUT_T; c ++){
//				for (int k = 0; k < CHANNEL_OUT_T; k ++) {
//					layer_3_downsample[c*CHANNEL_OUT + k][i][j] = out_buf_0[c][k][i][j];
//				}
//			}
//		}
//	}

	avgpool_concat(layer_3_downsample_out, out_buf_0, H_fmap_out, in_channels);
	LOOP_layer3_0_PGConv1:
	for (int c_out = 0; c_out < out_channels/OUT_CHANNEL_PARALLELISM; c_out ++) {
		for (int c_in = 0; c_in < in_channels; c_in++)
		{
			fp_conv1x1_tile(
					layer_3_downsample_out[c_out][c_in], downsample3_fp_1x1[conv_downsample_3_ptr][c_in],
					out_buf_t0_input,
	                c_in, in_channels, H_fmap_in, stride, padding
			);
		...
		bn1(
				layer_3_downsample_out, out_buf_t0_input,

				layer3_downsample_bn_weight_fix[c_out],
				layer3_downsample_bn_bias_fix[c_out],
	...
	for (int c_out = 0; c_out < out_channels/OUT_CHANNEL_PARALLELISM; c_out ++) {
		...
		conv_weight_ptr += 1;
		bn_relu_shortcut(
				layer_3_downsample_out, out_buf_t0,
...
#pragma HLS PIPELINE
			for (int c = 0; c < CHANNEL_OUT/CHANNEL_OUT_T; c ++){
				for (int k = 0; k < CHANNEL_OUT_T; k ++) {
					out_buf_0[c][k][i][j] = layer_3_downsample_out[c][k][i][j];
				}
			}
		}
	}
