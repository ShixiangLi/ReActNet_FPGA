...
/* 1-bit Quantization function  */
inline uint1 to1bit(FIX_FM_acc x)
{
    return (x >= 0) ? 1 : 0;
}
...
void quant_and_pack(
		...
		uint64 buffer[CHANNEL_IN][WIDTH][WIDTH],
		int H_fmap,
		int in_channels
)
{
...
#pragma HLS PIPELINE
				...
				for (int ch_offset=0; ch_offset<CHANNEL_OUT_T; ch_offset++){
					...
					if (ch < in_channels) {
						val = to1bit(raw_input);
					} else {
						val = 0;
					}
					buffer[0][row][col][63 - ch] = val;
				}
			}
		}
	}
...
#pragma HLS PIPELINE
			for (int ch = 0; ch < OUT_CHANNEL_PARALLELISM; ch ++) {
				out_buf[channel_tile][ch][row][col] = bn_weight[ch]*block_t0[ch][row][col] + bn_bias[ch];
			}
		}
	}
}
...
#pragma HLS PIPELINE
			...
			for (int ch_offset=0; ch_offset<BN_CHANNEL_PARALLELISM; ch_offset++){
				out_feature_t0[ch_offset] = block_t0[ch_offset][i][j];
			}
...
#pragma HLS PIPELINE
						for (int channel_pt = 0; channel_pt < BN_CHANNEL_PARALLELISM; channel_pt ++){
							if (ii + jj == 0) {
								out_feature[channel_pt] = inputs[tile][channel_pt][i*2 + ii][j*2 + jj];;
							} else {
								out_feature[channel_pt] += inputs[tile][channel_pt][i*2 + ii][j*2 + jj];;
							}
						}
